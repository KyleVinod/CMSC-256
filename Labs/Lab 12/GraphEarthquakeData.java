...
public class GraphEarthquakeData {
    ...
    public static void main(String[] args) throws Exception {
        ...
        GraphComparator list = new GraphComparator();
        eq_list.sort(list);
        List<EarthquakeUSGS> newList = eq_list.subList(0, 100);
        /* TODO:
        ...
        for (int i = 0; i < newList.size(); i++) {
            graph.addVertex(newList.get(i).getTitle(), newList.get(i).getTitle());
            if (newList.get(i).getMagnitude() > 6) {
                graph.getVertex(newList.get(i).getTitle()).setColor("red");
            }
            graph.getVisualizer(newList.get(i).getTitle()).setLocation(newList.get(i).getLongit(), newList.get(i).getLatit());
        }
        ...
        for (int i = 0; i < newList.size(); i++) {
            double calc = 501;
            if (i < newList.size() - 1) {
                calc = calcDistance(newList.get(i).getLatit(), newList.get(i).getLongit(), newList.get(i + 1).getLatit(), newList.get(i + 1).getLongit());
                if (calc <= 500 && i < newList.size() - 1) {
                    graph.addEdge(newList.get(i).getTitle(), newList.get(i + 1).getTitle());
                }
            }
        }
         ...
         */
        for (int i = 0; i < newList.size(); i++) {
            graph.getVisualizer(newList.get(i).getTitle()).setLocation(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
        }
